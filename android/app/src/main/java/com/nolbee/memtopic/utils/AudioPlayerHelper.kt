...
object AudioPlayerHelper {
    ...
    private fun changeMp3Bitrate(audioFile: File, context: Context): File {
        ...
        val cmd =
            "-i ${audioFile.absolutePath} -c:a libmp3lame -q:a 2 ${base64Fixed.absolutePath}"
    ...
    private fun cutIntervalSound(duration: Duration, context: Context): File {
        val dm = duration.toMillis()
        ...
        val cmd =
            "-i ${intervalSoundFile.absolutePath} -t ${dm}ms -c:a libmp3lame -q:a 2 ${intervalSoundFileShort.absolutePath}"
        val session = FFmpegKit.execute(cmd)
        assert(ReturnCode.isSuccess(session.returnCode))  // TODO: Raise exception if and handle it
    ...
    private fun concatAudioFiles(f1: File, f2: File, context: Context): File {
        ...
        val cmd =
            "-i \"concat:${f1.absolutePath}|${f2.absolutePath}\" -c copy ${mergedFile.absolutePath}"
        val session = FFmpegKit.execute(cmd)
        assert(ReturnCode.isSuccess(session.returnCode))  // TODO: Raise exception if and handle it
        return mergedFile
    }

    fun appendIntervalSound(audioBase64: String, context: Context): File {
        val ttsSoundFile = saveBase64ToMp3(audioBase64, context)
        val ttsSoundFileFixedBitrate = changeMp3Bitrate(ttsSoundFile, context)
        val duration = calculateAudioDuration(ttsSoundFileFixedBitrate)
